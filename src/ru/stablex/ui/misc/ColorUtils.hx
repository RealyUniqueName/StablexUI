...
class ColorUtils
{
    ...
    inline static public function getRGBA(Color:Int, Results:RGBA = null):RGBA
    {
        ...
        var alpha:Float = roundDecimal(((Color >> 24) & 0xFF) / 255, 4);
    ...
    inline static public function getHSBA(Color:Int, Results:HSBA = null):HSBA
    {
        ...
        brightness = roundDecimal(dmax, 4);
        saturation = 0;
        hue = 0;

        if (dmax != 0)
        {
            saturation = roundDecimal(range / dmax, 4);
        }
        if(saturation != 0)
        {
            if (red == dmax)
    ...
    static public function getRandomColor(Min:Int = 0, Max:Int = 255, Alpha:Int = 255):Int
    {
        ...
        if (Max > 255)
        {
            trace("Color: getRandomColor - max value too high");
            return getColor24(255, 255, 255);
        }

        if (Min > Max)
        {
            trace("Color: getRandomColor - min value higher than max");
            return getColor24(255, 255, 255);
        }
    ...
    inline static public function getComplementHarmony(Color:Int):Int
    {
        ...
        var opposite:Int = wrapValue(Std.int(hsv.hue), 180, 359);
    ...
    static public function getAnalogousHarmony(Color:Int, Threshold:Int = 30):Harmony
    {
        ...
        {
            trace("Color Warning: Invalid threshold given to getAnalogousHarmony()");
        }

        var warmer:Int = wrapValue(Std.int(hsv.hue), 359 - Threshold, 359);
        var colder:Int = wrapValue(Std.int(hsv.hue), Threshold, 359);
    ...
    static public function getSplitComplementHarmony(Color:Int, Threshold:Int = 30):Harmony
    {
        ...
        if (Threshold >= 359 || Threshold <= 0)
        {
            trace("Color: Invalid threshold given to getSplitComplementHarmony()");
        }

        var opposite:Int = wrapValue(Std.int(hsv.hue), 180, 359);

        var warmer:Int = wrapValue(Std.int(hsv.hue), opposite - Threshold, 359);
        var colder:Int = wrapValue(Std.int(hsv.hue), opposite + Threshold, 359);

        trace("hue: " + hsv.hue + " opposite: " + opposite + " warmer: " + warmer + " colder: " + colder);
    ...
    inline static public function getTriadicHarmony(Color:Int):TriadicHarmony
    {
        ...
        var triadic1:Int = wrapValue(Std.int(hsv.hue), 120, 359);
        var triadic2:Int = wrapValue(triadic1, 120, 359);
    ...
    static public function HSVtoRGBA(H:Float, S:Float, V:Float, Alpha:Int = 255):Int
    {
        ...
        else
        {
            ...
            switch (Std.int(H))
            {
                ...
                default:
                    trace("Color: HSVtoRGB: Unknown color");
            }
        }
     ...
     */
    private static function wrapValue(value:Int, amount:Int, max:Int):Int
    {
        var diff:Int;
     ...
     */
    inline static private function bound(Value:Float, Min:Float, Max:Float):Float
    {
